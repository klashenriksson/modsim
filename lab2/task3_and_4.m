...
final_net = stoch_sim_over_network(g, mu, beta, 0, 100, 0.02);
figure;
...
function final_net = stoch_sim_over_network(net,mu,beta, t_start, t_end, dt)
    figure;
    ...
    for iter = 2:max_iters
        ...
        final_net = diffuse_network_iteration(final_net);
        t(iter) = t(iter-1) + dt;
...
function final_net = diffuse_network_iteration(net)
    p = zeros(size(net.Nodes.Population, 1), 1);
    new_i = p;
    new_s = p;
    for n = 1:net.numnodes
        curr_neighbors = neighbors(net,n);
